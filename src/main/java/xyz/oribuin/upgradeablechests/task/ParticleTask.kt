...
class ParticleTask(private val plugin: UpgradeableChests) : BukkitRunnable() {
    ...
    override fun run() {
        this.plugin.getManager(DataManager::class.java).cachedChests.forEach { entry ->
            val loc = entry.value.location.block.location.clone().add(0.5, 0.5, 0.5)

            val transitionColor = Particle.DustTransition(Color.fromRGB(188, 79, 156), Color.fromRGB(248, 7, 89), 1f)
            loc.world?.spawnParticle(Particle.DUST_COLOR_TRANSITION, loc, 3, 0.0, 0.3, 0.0, transitionColor)
        }
    }
